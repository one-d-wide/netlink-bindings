...
```rust
PushOpNewlinkDoReply::new(&mut vec)
  ;
```
...
```rust
let iter = OpNewlinkDoRequest::new(buf);
for attr in iter {
  match attr {
    Ifname(val) => {}, // &CStr
    NetNsPid(val) => {}, // u32
    NetNsFd(val) => {}, // u32
    TargetNetnsid(val) => {}, // i32
    LinkNetnsid(val) => {}, // i32
    Linkinfo(iter) => {
      for attr in iter {
        match attr {
    ...
    GsoMaxSegs(val) => {}, // u32
    GroMaxSize(val) => {}, // u32
    GsoIpv4MaxSize(val) => {}, // u32
    GroIpv4MaxSize(val) => {}, // u32
    AfSpec(iter) => {
      for attr in iter {
        match attr {
          Inet(iter) => {
          ...
          Inet6(iter) => {
            for attr in iter {
              match attr {
                ...
                Stats(val) => {}, // &[u8]
                Mcast(val) => {}, // &[u8]
                Cacheinfo(val) => {}, // PushIflaCacheinfo
                Icmp6stats(val) => {}, // &[u8]
                Token(val) => {}, // &[u8]
                AddrGenMode(val) => {}, // u8
                RaMtu(val) => {}, // u32
...
```rust
let iter = OpNewlinkDoReply::new(buf);
// No attributes
```
...
```rust
PushOpDellinkDoReply::new(&mut vec)
  ;
```
...
```rust
let iter = OpDellinkDoRequest::new(buf);
for attr in iter {
  match attr {
    Ifname(val) => {}, // &CStr
  }
...
```rust
let iter = OpDellinkDoReply::new(buf);
// No attributes
```
...
```rust
PushOpGetlinkDoReply::new(&mut vec)
  .push_address(val) // &[u8]
  .push_broadcast(val) // &[u8]
  .push_ifname(val) // &CStr
  .push_ifname_bytes(val) // &[u8]
...
```rust
let iter = OpGetlinkDoRequest::new(buf);
for attr in iter {
  match attr {
    Ifname(val) => {}, // &CStr
    AltIfname(val) => {}, // &CStr
...
```rust
let iter = OpGetlinkDoReply::new(buf);
for attr in iter {
  match attr {
    Address(val) => {}, // &[u8]
    Broadcast(val) => {}, // &[u8]
      ...
      for attr in iter {
        match attr {
          ...
          Info(iter) => {
            for attr in iter {
              match attr {
                Mac(val) => {}, // PushIflaVfMac
                Vlan(val) => {}, // PushIflaVfVlan
                TxRate(val) => {}, // PushIflaVfTxRate
                Spoofchk(val) => {}, // PushIflaVfSpoofchk
                LinkState(val) => {}, // PushIflaVfLinkState
                Rate(val) => {}, // PushIflaVfRate
                RssQueryEn(val) => {}, // PushIflaVfRssQueryEn
                Stats(iter) => {
                  for attr in iter {
                    match attr {
                      RxPackets(val) => {}, // u64
                ...
                Trust(val) => {}, // PushIflaVfTrust
                IbNodeGuid(val) => {}, // PushIflaVfGuid
                IbPortGuid(val) => {}, // PushIflaVfGuid
                VlanList(iter) => {
                  for attr in iter {
                    match attr {

                      // Attribute may repeat multiple times (treat it as array)
                      Info(val) => {}, // PushIflaVfVlanInfo
                    }
                  }
                },
                Broadcast(val) => {}, // &[u8]
    ...
    Stats64(val) => {}, // PushRtnlLinkStats64
    VfPorts(iter) => {
      for attr in iter {
        match attr {
        }
    ...
    Carrier(val) => {}, // u8
    PhysPortId(val) => {}, // &[u8]
    CarrierChanges(val) => {}, // u32
    PhysSwitchId(val) => {}, // &[u8]
    LinkNetnsid(val) => {}, // i32
    PhysPortName(val) => {}, // &CStr
    ProtoDown(val) => {}, // u8
    GsoMaxSegs(val) => {}, // u32
    GsoMaxSize(val) => {}, // u32
    Pad(val) => {}, // &[u8]
    ...
    NewNetnsid(val) => {}, // i32
    TargetNetnsid(val) => {}, // i32
    CarrierUpCount(val) => {}, // u32
    CarrierDownCount(val) => {}, // u32
    NewIfindex(val) => {}, // i32
    MinMtu(val) => {}, // u32
    MaxMtu(val) => {}, // u32
    PropList(iter) => {
    ...
    PermAddress(val) => {}, // &[u8]
    ProtoDownReason(val) => {}, // &CStr
    ParentDevName(val) => {}, // &CStr
    ParentDevBusName(val) => {}, // &CStr
    GroMaxSize(val) => {}, // u32
    TsoMaxSize(val) => {}, // u32
    TsoMaxSegs(val) => {}, // u32
    Allmulti(val) => {}, // u32
    ...
    // EDT offload horizon supported by the device (in nsec).
    MaxPacingOffloadHorizon(val) => {}, // u32
    NetnsImmutable(val) => {}, // u8
  }
}
```
...
```rust
PushOpGetlinkDumpReply::new(&mut vec)
  .push_address(val) // &[u8]
  .push_broadcast(val) // &[u8]
  .push_ifname(val) // &CStr
  .push_ifname_bytes(val) // &[u8]
...
```rust
let iter = OpGetlinkDumpRequest::new(buf);
for attr in iter {
  match attr {
    TargetNetnsid(val) => {}, // i32
...
```rust
let iter = OpGetlinkDumpReply::new(buf);
for attr in iter {
  match attr {
    Address(val) => {}, // &[u8]
    Broadcast(val) => {}, // &[u8]
...
```rust
PushOpSetlinkDoReply::new(&mut vec)
  ;
```
...
```rust
let iter = OpSetlinkDoRequest::new(buf);
for attr in iter {
  match attr {
    Address(val) => {}, // &[u8]
    Broadcast(val) => {}, // &[u8]
...
```rust
let iter = OpSetlinkDoReply::new(buf);
// No attributes
```
...
```rust
PushOpGetstatsDoReply::new(&mut vec)
  .push_link_64(val) // PushRtnlLinkStats64
  .push_link_xstats(val) // &[u8]
  .push_link_xstats_slave(val) // &[u8]
  .nested_link_offload_xstats()
    .push_cpu_hit(val) // &[u8]
    .array_hw_s_info()
      .entry_nested()
        .push_request(val) // u8
...
```rust
let iter = OpGetstatsDoRequest::new(buf);
// No attributes
```
...
```rust
let iter = OpGetstatsDoReply::new(buf);
for attr in iter {
  match attr {
    Link64(val) => {}, // PushRtnlLinkStats64
    LinkXstats(val) => {}, // &[u8]
    LinkXstatsSlave(val) => {}, // &[u8]
    LinkOffloadXstats(iter) => {
      for attr in iter {
        match attr {
          CpuHit(val) => {}, // &[u8]
          HwSInfo(iter) => {
